...
import { isValidPlan, isValidRenewalPlan } from "../utils/validations/subscription";
...
export class SubscriptionService implements ISubscriptionService {
    ...
    async renewRazorpayOrder(userId: string): Promise<IRazorpayOrder> {
        ...
        const amountRupees = currentSubscription.billing === 'yearly' ? plan.yearlyAmount : plan.monthlyAmount;
        const amount = Math.round(amountRupees * 100);
        const order = await razorpayOrderCreat({
            amount,
            currency: 'INR',
            receipt: `rcpt_${Date.now()}`,
            notes: { planId: plan.id!.toString(), billing: currentSubscription.billing },
        })
        await this.paymentService.create({
            status: "PENDING",
            amount,
    ...
    async renewVerifyPayment(data: IVerifyPaymentDTO, userId: string): Promise<void> {
        const generatedSig = generatedSignature(data.razorpay_order_id, data.razorpay_payment_id)
        const isValid = verifySignature(generatedSig, data.razorpay_signature)
        if (!isValid) {
            throw new CustomError(ResponseMessages.VALIDATION_FAILED, HttpStatusCode.BAD_REQUEST)
        ...
        isValidRenewalPlan(existingSubscription)
        ...
        const start = new Date(existingSubscription.endAt);
        const end = new Date(start);
        if (data.billing === 'yearly') {
            end.setFullYear(end.getFullYear() + 1);
        } else {
