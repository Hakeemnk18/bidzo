...
const CategoryCreateModal = ({ isOpen, onClose, URL, categoryName, description }: CategoryCreateModalProps) => {
    if (!isOpen) return null


    const [formData, setFormData] = useState({ categoryName: categoryName, description: description });
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            setLoading(true)
            try {
                const res = await instance.post<ResGetToken>(URL, {
